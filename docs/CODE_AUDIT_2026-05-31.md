...
| H7 | DM auto-reply sends welcome on **every** inbound message, not just first contact — spam risk + burns budget | `apps/web/lib/meta/webhooks.ts:193` | ✅ fixed — `hasWelcomeDmBeenSentToSender()` checks for a prior `welcome_dm` event to this sender before replying |
| H8 | Scheduled-post status orphans at `publishing` if `markPostPublished()` fails — post is live externally but stuck in app | `apps/web/lib/inngest/functions/publish-scheduled-post.ts:27` | ✅ fixed — split try/catch: publish failure → `markPostFailed`; DB update failure after live publish → log + `published_unconfirmed`, never `markPostFailed` |
| H9 | `getAuthOrgId()` throws raw `Error("Not signed in")` — API routes return 500 instead of 401 JSON | `apps/web/lib/tenant.ts:19` | ✅ fixed — `AuthError` class added; `requireTenant()` helper in `lib/api.ts` catches it and returns 401 JSON; all JSON API routes updated |
| H10 | Canva access token stored in global cookie, not tenant-keyed — switching orgs in same browser uses wrong tenant's token | `apps/web/app/api/canva/oauth/callback/route.ts:53` | ✅ fixed — cookie renamed `canva_access_token_{tenantId}`; designs/create reads the tenant-keyed cookie |
| H11 | Post-image-generation `fetch(first.url)` has no SSRF guard or timeout | `apps/web/lib/openai/images.ts:40` | ✅ fixed — `isSafeFetchUrl()` check before fetch; 30s `AbortSignal.timeout` added |
| H12 | Meta + Clerk webhook routes have no rate limiter — invalid-signature spam is cheap DoS | `apps/web/app/api/webhooks/meta/route.ts:20`, `apps/web/app/api/webhooks/clerk/route.ts:16` | ✅ fixed — 100 req/min per IP via Upstash sliding window on both GET+POST (meta) and POST (clerk) |
| H13 | When `clerkConfigured=false`, middleware is a no-op and **all** API routes become unauthenticated — silent total auth bypass on misconfig | `apps/web/middleware.ts:16` | ✅ fixed — unconfigured middleware now returns 503 on non-public routes in production; local-dev pass-through preserved |
| H14 | SSRF guard checks hostname string only — no DNS resolution, no redirect blocking → public hostname resolving to `169.254.169.254` still reaches metadata server | `apps/web/lib/url-safety.ts:41-61` | ✅ fixed — `isSafeFetchUrlAsync` added: resolves hostname via `dns.lookup` and rejects private IPs; both callers upgraded + `redirect: "error"` added |
